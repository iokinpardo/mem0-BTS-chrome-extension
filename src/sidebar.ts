...
(function () {
  ...
  function createSidebar(): void {
    ...
    inputContainer.innerHTML = `
      <form id="memoryQueryForm" class="memory-query" novalidate>
        <label for="memoryQueryInput" class="visually-hidden">Ask about your memories</label>
        <textarea
          id="memoryQueryInput"
          class="query-input"
        ...
        <div class="query-controls">
          <label for="memoryCategorySelect" class="visually-hidden">Memory category filter</label>
          <select id="memoryCategorySelect" class="query-select">
            <option value="all">All memories</option>
            <option value="simple">Simple memories</option>
            <option value="multiple">Multiple memories</option>
          </select>
          <button type="submit" id="memoryQueryBtn" class="query-button">Run query</button>
        </div>
      </form>
      <div id="memoryQueryResult" class="query-result" hidden></div>
    `;
  ...
  function setupEventListeners(
    ...
    const queryForm = sidebarContainer.querySelector('#memoryQueryForm') as HTMLFormElement;
    const queryInput = sidebarContainer.querySelector('#memoryQueryInput') as HTMLTextAreaElement;
    const queryButton = sidebarContainer.querySelector('#memoryQueryBtn') as HTMLButtonElement;
    ...
    categorySelect?.addEventListener('change', () => {
      ...
      if (latestQueryContext && latestQueryContext.query === currentQuery) {
        renderMemoryQueryResult({
          category: categorySelect.value,
          resultContainer: queryResult,
          query: currentQuery,
        });
      }
    });
  ...
  function handleMemoryQuery(options: {
    ...
    const { queryInput, categorySelect, trigger, resultContainer } = options;
    if (!queryInput || !categorySelect || !resultContainer) {
      return;
    }
  ...
  function renderMemoryQueryResult(options: {
    category: string;
    resultContainer: HTMLElement | null;
    query: string;
    message?: string;
  }): void {
    const { category, resultContainer, query, message } = options;

    if (!resultContainer || !latestQueryContext || latestQueryContext.query !== query) {
      return;
    }

    const filteredMemories = filterMemoriesByCategory(latestQueryContext.memories, category);
    const hasAnyMemories = latestQueryContext.memories.length > 0;

    const noMemoriesMessage =
      !filteredMemories.length && category !== 'all' && hasAnyMemories
        ? 'No memories matched the selected category.'
        : undefined;

    const defaultMessage = !hasAnyMemories ? 'No memories matched your query.' : undefined;

    updateQueryResultState('success', {
      resultContainer,
      answer: latestQueryContext.answer ?? null,
  ...
  function filterMemoriesByCategory(
    ...
    const keywords =
      normalizedCategory === 'simple'
        ? ['simple', 'single', 'basic']
        : ['multiple', 'multi', 'complex', 'compound'];
  ...
  function updateQueryResultState(
    ...
    if (state === 'idle') {
      ...
      resultContainer.classList.remove('error', 'loading', 'empty');
      return;
    }
    ...
    if (answer) {
      html += `<p class="query-answer">${escapeHtml(answer)}</p>`;
    }
    ...
    if (memories && memories.length > 0) {
      const listItems = memories
        .map(memory => {
          ...
          const categoryLabels = (memory.categories || [])
            .map(cat => `<span class="memory-category">${escapeHtml(cat)}</span>`)
            .join('');
          const categoriesMarkup = categoryLabels
            ? `<div class="memory-categories">${categoryLabels}</div>`
            : '';
          return `<li class="query-memory-item"><p>${escapeHtml(preview)}</p>${categoriesMarkup}</li>`;
        })
        .join('');

      html += `<div class="query-supporting"><h4>Supporting memories</h4><ul>${listItems}</ul></div>`;
    }

    if ((!memories || memories.length === 0) && noMemoriesMessage) {
      html += `<p class="query-status muted">${escapeHtml(noMemoriesMessage)}</p>`;
    }

    if (message) {
      html += `<p class="query-status muted">${escapeHtml(message)}</p>`;
    }

    if (!html) {
      html = '<p class="query-status muted">No matching memories found.</p>';
    }
